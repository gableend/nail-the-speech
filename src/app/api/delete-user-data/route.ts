...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userId) {
      ...
      // For Facebook webhook integration
      targetUser = await prisma.user.findFirst({
        where: {
          OR: [
            { externalId: facebook_user_id },
            { clerkId: { contains: facebook_user_id } }
          ]
        },
        include: { speeches: true }
      });
    ...
    // Delete all user speeches first
    await prisma.speech.deleteMany({
      where: { userId: targetUser.clerkId }
    });
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="background-color: #f8f9fa; padding: 16px; border-radius: 8px; margin: 20px 0;">
                ...
                <ul>
                  ...
                  <li>All ${targetUser.speeches.length} speech(es) you created</li>
    ...
    return NextResponse.json({
      ...
      speechesDeleted: targetUser.speeches.length,
      confirmationEmailSent: !!targetUser.email
    });
